...
if ( ! class_exists( '\E20R\Utilities\E20R_Background_Process' ) ) {
	...
	abstract class E20R_Background_Process extends E20R_Async_Request {
		...
		public function __construct() {
			...
			add_filter( 'cron_schedules', array( $this, 'schedule_cron_healthcheck' ), 10 );
		}
		...
		protected function handle() {
			...
			$utils->log(
				'Batch operation done. Time: ' .
				( $this->time_exceeded() ? 'Yes' : 'No' ) .
				' Memory: ' . ( $this->memory_exceeded() ? 'Yes' : 'No' ) .
				' Queue complete: ' .
				( $this->is_queue_empty() && empty( $this->batch ) ? 'Yes' : 'No' )
			);
